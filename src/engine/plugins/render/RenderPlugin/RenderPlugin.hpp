...
SYSTEM(RenderSpriteSys, With<GlobalPosition, Sprite, Size>, On<Render>) {
    ITER(view) {
        ...
        for (uint i = 0; i < view.count(); i++) {
            ...
            const float diffx = std::abs(sizes[i].width - static_cast<float>(sprites[i].rect.width));
            const float diffy = std::abs(sizes[i].height - static_cast<float>(sprites[i].rect.height));
