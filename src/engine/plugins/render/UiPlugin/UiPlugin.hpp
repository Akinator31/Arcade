...
struct AnimatedSprite : Required<Sprite, HoveredSensorComponent> {
    ...
    AnimatedSprite(const Sprite &base, const Sprite &hover, const Sprite &click,
                   const bool enabled = true) : Required(), base(base), hover(hover), click(click), enabled(enabled) {
    }
};
...
struct ImageOnHoverComponent : Required<HoveredSensorComponent> {
    ...
    ImageOnHoverComponent(const Sprite &image, const Sprite &imageOnHover) : Required{}, image(image),
...
struct Text {
    ...
    static void onRemove(ecs::World &, ecs::Entity, const Text *value) {
        free(value->text);
    }
};
